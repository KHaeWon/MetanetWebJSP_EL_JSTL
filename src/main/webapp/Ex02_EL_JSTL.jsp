...
<%
	...
	//해결방법 >> 1) 이렇게 request나 session으로 담아라. 그러면
...
<h3>EL</h3>
EL() : 자바 객체 출력 (★자바 객체에 대해서 직접 접근 불가): ${e}<br><!-- request나 session에 담아서 사용해야 한다. -->
EL() : ${e.getEmpno()}<br>

<h3>해결사</h3>
-----------------------------------------------------------<br>
1. 방법 : request객체 또는 session 객체에 담는다 good<br>	<!-- 2) 그러면 이렇게 사용 가능하다. -->
EL : ${requestScope.emp}<br>
EL : ${requestScope.emp.empno}<br><!-- el은 멤버필드명만 적어준다. 그러면 내부적으로 알아서! get 호출 -->
...
<c:set var="m" value="<%=e%>"></c:set>	<!-- c:set >> 변수 만들기, m이라는 변수가 Emp 객체 e의 주소값을 가짐 -->
<br>
